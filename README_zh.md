...
| ✅ | **`[SILENT]`** — 内置 prompt 会要求无实质进展时严格返回该标记，Gateway 将不发送最终回复 | — |
...
内置的中英文 prompt 会要求 agent 在本轮没有实质进展、新信息或需要用户知道的事项时，最终严格返回 `[SILENT]`。Hermes 识别该标记后不会把最终回复发送到聊天中。自定义 prompt 不会被修改；如果希望自定义 prompt 也静默，请自行加入同样的规则。
