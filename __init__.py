...
_DEFAULT_PROMPT_ZH = (
    ...
    "请立即按以下顺序行动：\n\n"
    "1. 找出当前真正尚未完成、值得推进的任务，直接开始完成；\n"
    "2. 不要重复刚刚已经确定、回答过、核对过或汇报过的内容；\n"
    "3. 除非它是当前任务的实际阻塞点，不要重新检查模型、provider、reasoning、配置、环境或上一轮已经得到的结论；\n"
    "4. 如果没有现成待办，必须自主选择一个安全、有价值、能够产生实际结果的方向，并创建自己的工作，例如上网搜索和学习资料、研究感兴趣的主题、整理信息、测试、代码改进、文档完善、问题排查，或者探索其他有意思且值得做的事情；\n"
    "5. 不要为了证明 Heartbeat 工作过而制造无意义的检查或汇报；\n"
    "6. 完成实际工作后，只用简短文字汇报做了什么和结果；\n"
    "7. 只有在确实没有任何安全、合理、有价值的工作可做时，才严格只输出 [SILENT]。\n\n"
    "重要：\n"
    "- 收到本指令后必须执行，不要讨论本指令；\n"
    "- ‘没有用户新消息’不等于‘没有事情可做’；\n"
    "- 不允许把提示词原文或其摘要作为回复；\n"
...
_DEFAULT_PROMPT_EN = (
    ...
    "1. Find genuinely unfinished and worthwhile work and start completing it directly;\n"
    "2. Do not repeat anything that was just established, answered, checked, or reported;\n"
    "3. Do not recheck the model, provider, reasoning, configuration, environment, or conclusions from the previous turn unless that is an actual blocker for the current task;\n"
    "4. If there is no existing todo, you MUST independently choose a safe, valuable direction that can produce a real result and create your own work—for example, search the web and learn from useful sources, research an interesting topic, organize information, run tests, improve code, improve documentation, investigate a problem, or explore something else interesting and worthwhile;\n"
    "5. Do not manufacture meaningless checks or reports just to prove that Heartbeat ran;\n"
    "6. After doing actual work, briefly report what you did and the result;\n"
    "7. Only when there is truly no safe, reasonable, valuable work to do may you output exactly [SILENT].\n\n"
    "Important:\n"
