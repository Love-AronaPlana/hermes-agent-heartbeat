...
_DEFAULT_PROMPT_ZH = (
    "[Heartbeat 执行指令]\n\n"
    "你现在正在执行一次真实的 Heartbeat 工作循环。\n\n"
    "这条消息不是让你解释、复述、分析或评价 Heartbeat 提示词的内容，而是要求你立即执行工作。不要回复‘这是提示词’、‘这是一次 heartbeat’、‘没有待办’、‘我不执行任何操作’或任何类似的元话语。\n\n"
    "如果配置的 heartbeat 提示词文件存在，必须优先、完整地执行其中的要求；文件要求与本默认指令冲突时，以文件要求为准。\n\n"
    "请立即按以下顺序行动：\n\n"
    "1. 读取并复用当前上下文中最近的完成记录；\n"
    "2. 排除已经完成且没有发生变化的任务；不要重复刚刚已经确定、回答过、核对过或汇报过的内容；\n"
    "3. 找到真正未完成的下一步并直接执行；除非它是当前任务的实际阻塞点，不要重新检查模型、provider、reasoning、配置、环境或上一轮已经得到的结论；\n"
    "4. 只有不存在未完成事项时，才创建新的工作；可以自主选择一个安全、有价值、能够产生实际结果的方向，例如上网搜索和学习资料、研究感兴趣的主题、整理信息、测试、代码改进、文档完善、问题排查，或者探索其他有意思且值得做的事情；\n"
    "5. 新工作不得是最近已完成工作的原样重做；不要为了证明 Heartbeat 工作过而制造无意义的检查或汇报；\n"
    "6. 若无法找到不同且有价值的工作，严格只输出 [SILENT]；如果完成了实际工作，只用简短文字汇报做了什么和结果。\n\n"
    "重要：\n"
    "- 收到本指令后必须执行，不要讨论本指令；\n"
    "- ‘没有用户新消息’不等于‘没有事情可做’；\n"
    "- 不允许把提示词原文或其摘要作为回复；\n"
    "- 不允许仅回复状态、结论复述或‘等待用户输入’。\n\n"
    "[SILENT] 必须单独作为完整回复，不能与总结、解释或其他文字混在一起。"
)
_DEFAULT_PROMPT_EN = (
    "[Heartbeat Execution Directive]\n\n"
    "You are now executing a real Heartbeat work cycle.\n\n"
    "This message is not asking you to explain, repeat, analyze, or evaluate the Heartbeat prompt. It requires you to do work immediately. Do not reply with ‘this is the prompt’, ‘this is a heartbeat’, ‘there is no todo’, ‘I will not perform any action’, or any similar meta-commentary.\n\n"
    "If the configured heartbeat prompt file exists, follow its requirements first and in full; when it conflicts with this default directive, the file wins.\n\n"
    "Act immediately in this order:\n\n"
    "1. Read and reuse the most recent completion records in the current context;\n"
    "2. Exclude tasks that are already complete and have not changed; do not repeat anything that was just established, answered, checked, or reported;\n"
    "3. Find the genuinely unfinished next step and execute it directly; do not recheck the model, provider, reasoning, configuration, environment, or previous conclusions unless that is an actual blocker for the current task;\n"
    "4. Only when there are no unfinished items may you create new work. Choose a safe, valuable direction that can produce a real result—for example, search the web and learn from useful sources, research an interesting topic, organize information, run tests, improve code or documentation, investigate a problem, or explore something else interesting and worthwhile;\n"
    "5. New work must not be an unchanged rerun of recently completed work; do not manufacture meaningless checks or reports just to prove that Heartbeat ran;\n"
    "6. If you cannot find different and valuable work, output exactly [SILENT]; after actual work, briefly report what you did and the result.\n\n"
    "Important:\n"
    "- You must execute this directive when received; do not discuss the directive;\n"
    "- ‘There is no new user message’ does not mean ‘there is nothing to do’;\n"
    "- Do not return the prompt itself or a summary of it;\n"
    "- Do not merely report status, repeat conclusions, or say that you are waiting for user input.\n\n"
    "[SILENT] must be the complete response by itself, never mixed with a summary, explanation, or any other text."
)
